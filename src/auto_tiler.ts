...
export class AutoTiler {
  ...
  toggle_floating(ext: Ext) {
    ...
    const center_floating = () => {
      const area = ext.monitor_work_area(focused.meta.get_monitor()); // usable screen space, excludes top bar

      const DOCK_HEIGHT = 101; // pixels the dock takes up at the bottom
      const usable_height = area.height - DOCK_HEIGHT; // space above the dock

      const ASPECT_RATIO = 16 / 10; // width divided by height
      const MAX_FRACTION = 0.8; // window can be at most 80% of the screen

      const max_width = area.width * MAX_FRACTION;
      const max_height = area.height * MAX_FRACTION;

      // Try fitting to max width first
      let width = max_width;
      let height = width / ASPECT_RATIO;

      // If that makes it too tall, fit to max height instead
      ...
      focused.move(ext, {
        x: Math.floor(area.x + (area.width - width) / 2), // horizontal center
        y: Math.floor(area.y + (usable_height - height) / 2), // vertical center, above the dock
        width,
        height,
      });
    };
